...
server {
    ...
    server_name 172.18.119.215;  # <--- 你的公网 IP
...
curl -k -I https://172.18.119.215/index.json     
# 期望看到：HTTP/1.1 200 OK 且 Cache-Control: no-store

# 传个测试包后测试下载：
curl -k -O https://172.18.119.215/packages/test-0.1.0.zip
```
## 发布通道（SSH/scp，不暴露上传接口）
```
# 开发机一次配置免密：
ssh-keygen -t ed25519
ssh-copy-id 用户@172.18.119.215
```
然后 publish.py 里 SERVER="用户@172.18.119.215"、REMOTE="/var/www/repo"，每次 python publish.py apps/ocr 即完成上架。
